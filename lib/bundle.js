  ...
  _createClass(Ball, [{
    ...
    value: function updateBall(paddle, lives, balls, gameData) {
      ...
      if (this.ballPos[1] - this.ballRadius < 0) {
        ...
        this.paddleHit.play();
        if (this.ballPos[0] + this.ballRadius > paddle.paddleX + paddle.paddleWidth / 2 && this.ballPos[0] - this.ballRadius < paddle.paddleX + paddle.paddleWidth) {
          ...
          for (var r = 0; r < this.cols; r++) {
            ...
            if (brick.status == 1) {
              if (ball.ballPos[0] + ball.ballRadius > brick.brickPos[0] && ball.ballPos[0] + ball.ballRadius < brick.brickPos[0] + brick.brickWidth && ball.ballPos[1] - ball.ballRadius > brick.brickPos[1] && ball.ballPos[1] - ball.ballRadius < brick.brickPos[1] + brick.brickHeight) {
                ...
                if (gameData.score == this.rows * this.cols + this.originalScore) {
                  this.won = true;
                }
              }
            }
...
var Game = function () {
  function Game(canvas, ctx) {
    ...
    this.gameData = { score: 0, lives: 3, paused: false, started: false, highScore: 0, stopped: false };
  ...
  _createClass(Game, [{
    ...
    value: function resetGame() {
      ...
      this.gameData = { score: 0, lives: 3, paused: false, started: true, highScore: oldHighScore, stopped: false };
    }
  }, {
    key: 'drawLives',
